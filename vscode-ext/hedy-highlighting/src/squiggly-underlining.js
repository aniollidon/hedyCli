    ...
    if (errors !== null) {
      for (let err of errors) {
        ...
        const diagnostic = new vscode.Diagnostic(
          new vscode.Range(i, error.start, i, error.end),
          error.message,
          severity,
        )
  ...
  provideCodeActions(document, range, context, token) {
    ...
    context.diagnostics.forEach(diagnostic => {
      if (diagnostic.code === 'hy-recomended-equal') {
        const fix = new vscode.CodeAction("Reemplaça 'is' per '='", vscode.CodeActionKind.QuickFix)
        ...
        const fix = new vscode.CodeAction('Converteix la comanda a minúscules', vscode.CodeActionKind.QuickFix)
        fix.edit = new vscode.WorkspaceEdit()
        ...
        const fix = new vscode.CodeAction("Reemplaça '=' per '=='", vscode.CodeActionKind.QuickFix)
        fix.edit = new vscode.WorkspaceEdit()
        fix.edit.replace(document.uri, diagnostic.range, '==')
        fix.diagnostics = [diagnostic]
        fix.isPreferred = true // Marca com a preferida aquesta solució
        ...
        const fix = new vscode.CodeAction('Elimina les cometes', vscode.CodeActionKind.QuickFix)
        fix.edit = new vscode.WorkspaceEdit()
        ...
        const fix = new vscode.CodeAction('Afegeix cometes', vscode.CodeActionKind.QuickFix)
        fix.edit = new vscode.WorkspaceEdit()
        const text = document.getText(diagnostic.range)
        const newtext = `"${text}"`
        fix.edit.replace(document.uri, diagnostic.range, newtext)
