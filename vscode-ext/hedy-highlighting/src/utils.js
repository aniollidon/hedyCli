...
function detectTypeConstant(text) {
  ...
  if (
    word.match(
      /^\b(?:C[0-9]|D[0-9]|E[0-9]|F[0-9]|G[0-9]|A[0-9]|B[0-9]|[1-6][0-9]|70|[1-9]|[A-G])\b$/
    )
  )
    return "note";
  ...
  if (
    word.match(
      /\b(blue|green|red|black|brown|gray|orange|pink|purple|white|yellow)\b/
    )
  )
    return "color";
  ...
  while (i < tokens.length) {
    ...
    if (
      ...
      result.push({
        ...
        pos: phrase[0].pos,
          ...
          phrase[phrase.length - 1].pos + phrase[phrase.length - 1].text.length,
        type: "function_usage",
        subphrase: phrase,
      });
    }
    ...
    // range _ to _ calls
      ...
      for (let j = i + 1; j < tokens.length && j < i + 4; j++) {
        if (
          ((j === i + 1 || j === i + 3) && !tokens[j].command) ||
          (j === i + 2 && tokens[j].command === "to_range")
        ) {
    ...
    // Function calls
      ...
      if (i + 2 < tokens.length && tokens[i + 2].tag === "command_with") {
        ...
        while (
          i < tokens.length &&
          nextargument &&
          (tokens[i].tag.startsWith("entity_variable") ||
            tokens[i].tag.startsWith("constant"))
        ) {
          if (
            i + 2 < tokens.length &&
            tokens[i + 1].tag === "command_comma_list"
          ) {
        ...
        result.push({
          ...
          pos: pos,
            ...
            phrase[phrase.length - 1].pos +
            phrase[phrase.length - 1].text.length,
          type: "function_usage",
          subphrase: phrase,
        });
      }
  ...
  while (i < tokens.length) {
    if (
      ...
      while (
        ...
        tokens[i + count].command &&
        search.has(tokens[i + count].command)
      ) {
...
function detectConditions(tokens) {
  ...
  while (j < tokens.length) {
    if (
      tokens[j].command === "not" &&
      j + 1 < tokens.length &&
      tokens[j + 1].command === "in"
    ) {
  ...
  while (i < tokens.length) {
    if (
      ...
      !tokens[i].command &&
      tokens[i + 1].command &&
      comparators.has(tokens[i + 1].text)
    ) {
...
function varDefinitionType(linetext, hasQuotes, define_var_operator, entities) {
  ...
  const despresIgual = define_var_operator.includes("=")
    ? linetext.indexOf("=") + 1
    : -1;
  const despresIs = define_var_operator.includes("is")
    ? linetext.indexOf(" is ") + 4
    : -1;
...
function enUnaLlista(text, pos, hasQuotes, define_var_operator) {
  ...
  let abansIgual = define_var_operator.includes("=")
    ? abans.lastIndexOf("=")
    : -1;
  let abansIs = define_var_operator.includes("is")
    ? abans.lastIndexOf(" is ")
    : -1;
  ...
  if (hasQuotes) {
    ...
    if (
      conteWith !== -1 &&
      !entreCometes(text, conteWith) &&
      conteCall !== -1 &&
      !entreCometes(text, conteCall)
    )
      return false;
  }
